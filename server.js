...
app.get('/api/places/autocomplete', async (req, res) => {
  const q = req.query.q;
  const url = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(q)}&types=address&components=country:us&key=${process.env.GOOGLE_API_KEY}`;
...
app.get('/api/places/details', async (req, res) => {
  ...
  const url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=geometry&key=${process.env.GOOGLE_API_KEY}`;
  const r = await fetch(url);
  const data = await r.json();
